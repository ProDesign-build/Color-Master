...
export default function Library() {
  ...
  const handleManualBackup = async () => {
    ...
    if (result === 'overwritten') alert("Success! Your linked backup file has been updated.");
        ...
        alert("The backup file was deleted from your device. Please select a new location.");
    ...
    <div className="h-full flex flex-col gap-4">
      <div className="flex justify-between items-center">
        <div className="flex p-1 bg-cream-100/80 backdrop-blur-sm rounded-xl border border-cream-200">
            <button onClick={() => handleTabChange('swatches')} className={`flex items-center gap-2 px-4 md:px-6 py-2.5 rounded-lg text-sm font-bold transition-all duration-300 ${activeTab === 'swatches' ? 'bg-navy-900 text-gold-500 shadow-md' : 'text-navy-800'}`}>
                <Palette size={16} /><span>Swatches</span>
            </button>
            <button onClick={() => handleTabChange('formulas')} className={`flex items-center gap-2 px-4 md:px-6 py-2.5 rounded-lg text-sm font-bold transition-all duration-300 ${activeTab === 'formulas' ? 'bg-navy-900 text-gold-500 shadow-md' : 'text-navy-800'}`}>
                <Droplet size={16} /><span>Formulas</span>
            </button>
        </div>
        <button onClick={() => setShowDataModal(true)} className="flex items-center gap-2 px-3 py-2 bg-white border border-gray-200 rounded-lg text-navy-800 hover:text-gold-600 shadow-sm transition-all">
            ...
            <div className="absolute inset-0 bg-white rounded-xl shadow-sm border border-cream-100 flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="p-4 border-b border-gray-100 bg-cream-50/50">
                    <div className="relative group max-w-md">
                        ...
                        <input type="text" placeholder="Search swatches..." value={swatchSearch} onChange={(e) => setSwatchSearch(e.target.value)} className="w-full pl-9 pr-8 py-2.5 bg-white border border-gray-200 rounded-lg text-sm focus:border-navy-900 outline-none transition-all placeholder:text-gray-400"/>
                    </div>
                </div>
                <div className="flex-grow overflow-y-auto p-4 custom-scrollbar pb-32 lg:pb-4">
                     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                        {filteredSwatches.map(swatch => (
                            <div key={swatch.id} onClick={() => toggleActive(swatch.id!)} className={`flex items-center justify-between p-3 rounded-xl border transition-all duration-300 group cursor-pointer ${activeId === swatch.id ? 'border-gold-500 bg-gold-50/10 shadow-md' : 'border-gray-100 bg-white'}`}>
                                <div className="flex items-center gap-3 flex-grow min-w-0">
                                    ...
                                    {editingId === swatch.id ? (<div className="flex items-center gap-1 flex-grow" onClick={e => e.stopPropagation()}><input className="bg-white border border-gold-500 rounded px-2 py-1 text-sm w-full outline-none" value={editName} onChange={(e) => setEditName(e.target.value)} autoFocus /><button onClick={() => saveRename(swatch.id!, 'swatch')} className="text-white bg-green-500 p-1 rounded shadow-sm"><Check size={16}/></button></div>) : (<div className="min-w-0 flex-grow"><div className="font-bold text-navy-900 text-sm flex items-center gap-2 truncate">{swatch.name}<button onClick={(e) => { e.stopPropagation(); startEditing(swatch.id!, swatch.name); }} className={`transition-all ${activeId === swatch.id ? 'opacity-100' : 'opacity-0 lg:group-hover:opacity-100'} text-gray-400`}><Edit2 size={12}/></button></div><div className="text-[10px] text-gray-500 font-mono uppercase">{swatch.hex}</div></div>)}
                                </div>
                                <div className={`flex gap-1 pl-2 transition-all duration-300 ${activeId === swatch.id ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none lg:group-hover:opacity-100 lg:group-hover:pointer-events-auto'}`}>
                                    <button onClick={(e) => { e.stopPropagation(); handleShareSwatch(swatch.name, swatch.hex); }} className="text-navy-800 p-2 rounded-lg hover:bg-cream-50"><Share2 size={16}/></button>
                                    <button onClick={(e) => { e.stopPropagation(); handleDownloadSwatch(swatch.name, swatch.hex); }} className="text-navy-800 p-2 rounded-lg hover:bg-cream-50"><Download size={16}/></button>
        ...
        {activeTab === 'formulas' && (
             <div className="absolute inset-0 bg-white rounded-xl shadow-sm border border-cream-100 flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="p-4 border-b border-gray-100 bg-cream-50/50">
                    <div className="relative group max-w-md">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
                        <input type="text" placeholder="Search formulas..." value={formulaSearch} onChange={(e) => setFormulaSearch(e.target.value)} className="w-full pl-9 pr-8 py-2.5 bg-white border border-gray-200 rounded-lg text-sm focus:border-navy-900 outline-none transition-all placeholder:text-gray-400" />
                    </div>
                </div>
                <div className="flex-grow overflow-y-auto p-4 custom-scrollbar pb-32 lg:pb-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {filteredFormulas.map(formula => (
                            <div key={formula.id} onClick={() => toggleActive(formula.id!)} className={`p-5 rounded-xl border transition-all duration-300 flex flex-col h-full cursor-pointer ${activeId === formula.id ? 'border-navy-500 bg-navy-50/10 shadow-md' : 'border-gray-100 bg-white'}`}>
                                <div className="flex justify-between items-start mb-3">
                                    <div className="flex-grow min-w-0 pr-2">
                                        ...
                                        <div className="text-[10px] text-gray-400 mt-2 uppercase tracking-wider font-medium flex gap-2"><span className="bg-gray-100 px-2 py-0.5 rounded text-gray-600">Batch: {formula.batchSize}{formula.unit}</span></div>
                                    </div>
                                    <button onClick={(e) => { e.stopPropagation(); deleteFormula(formula.id!); }} className={`p-2 rounded-lg transition-all ${activeId === formula.id ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none lg:group-hover:opacity-100 lg:group-hover:pointer-events-auto'} text-gray-300 hover:text-red-500`}><Trash2 size={18}/></button>
                                </div>
                                <div className="space-y-2 border-t border-gray-50 pt-3 mt-auto">
                                    {formula.pigments.map(p => (
                                        <div key={p.id} className="flex justify-between text-xs items-center group/item hover:bg-cream-50/50 p-1 rounded -mx-1 transition-colors">
                                            ...
                                            <span className="font-mono text-navy-900 font-bold bg-cream-100 px-1.5 py-0.5 rounded border border-cream-200">
                                                {formula.ratioMode === 'percentage' ? `${((p.ratio/100)*formula.batchSize).toFixed(1)}` : `${(p.ratio * (formula.batchSize / formula.pigments.reduce((s,x)=>s+(x.ratio || 0),0))).toFixed(1)}`}
    ...
    {showDataModal && (
        <div className="fixed inset-0 bg-navy-900/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4 animate-in fade-in duration-150">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
                <div className="bg-cream-50 p-5 border-b border-cream-100 flex items-center justify-between">
                    <div className="flex items-center gap-3"><Database className="text-navy-900" size={24} /><h3 className="font-serif font-bold text-lg text-navy-900">Library Management</h3></div>
                    <button onClick={() => setShowDataModal(false)} className="text-gray-400 hover:text-red-500"><X size={20} /></button>
                </div>
                <div className="p-6 space-y-6">
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <button onClick={handleManualBackup} disabled={isBackingUp} className={`flex flex-col items-center justify-center gap-3 p-4 rounded-xl border-2 border-dashed transition-all group col-span-2 ${hasLinkedFile ? 'border-green-200 bg-green-50 hover:bg-green-100' : 'border-gray-200 hover:border-navy-900 hover:bg-navy-50'}`}>
                                <div className={`p-3 rounded-full shadow-sm ${hasLinkedFile ? 'bg-white text-green-700' : 'bg-white text-navy-900'}`}>
                    ...
                    <div className="bg-yellow-50 p-3 rounded-lg border border-yellow-100 flex gap-2">
                        ...
                        <span className="text-[10px] text-yellow-800 font-medium leading-tight">Restore merges data into current library. No items are deleted.</span>
                    </div>
                    <button onClick={handleResetDatabase} className="w-full flex items-center justify-center gap-2 p-3 bg-red-50 text-red-600 border border-red-200 rounded-lg hover:bg-red-100 transition-colors font-bold text-sm uppercase tracking-wider"><Power size={16} /> Factory Reset</button>
                </div>
            </div>
        </div>
    )}
